...
def process_image(pc, ser):
    lower_bound = np.array([25,100,100])  # Color range for non-white balls filtering
    upper_bound = np.array([35,255,255])
    dispW = 640
    dispH = 360
    pc.preview_configuration.main.size = (dispW, dispH)
    ...
    while True:
        ...
        x_distances.sort()
        if len(x_distances) > 0:
            closest_distance = min(x_distances, default=None)
            if closest_distance is not None:
                print("closest distance:", closest_distance)
                t = threading.Thread(target=send_data, args=(ser, closest_distance))
                t.start()
        cv2.imshow("camera feed", frame)
        cv2.imshow('my Mask',my_mask)
        if cv2.waitKey(1) == ord('q'):
...
if name == 'main':
    ...
    ser = serial.Serial('/dev/ttyACM0', 9600, timeout=1)
    time.sleep(2)
    t1 = threading.Thread(target=process_image, args=(pc, ser))
    t2 = threading.Thread(target=get_position, args=(pc,))
    t1.start()
    t2.start()
    t1.join()
    t2.join()
...
def main():
    ...
    West = [[0,0,-1],[0,1,0],[1,0,0]]
    tags = {
        0: (np.array([[150],[0],[0]]), np.array(North)),
        1: (np.array([[450],[0],[0]]), np.array(North)),
        2: (np.array([[750],[0],[0]]), np.array(North)),
        3: (np.array([[1250],[0],[0]]), np.array(North)),
        4: (np.array([[1550],[0],[0]]), np.array(North)),
        5: (np.array([[1850],[0],[0]]), np.array(North)),
        6: (np.array([[2000],[0],[-150]]), np.array(East)),
        7: (np.array([[2000],[0],[-450]]), np.array(East)),
        8: (np.array([[2000],[0],[-750]]), np.array(East)),
        9: (np.array([[2000],[0],[-1250]]), np.array(East)),
        10: (np.array([[2000],[0],[-1550]]), np.array(East)),
        11: (np.array([[2000],[0],[-1850]]), np.array(East)),
        12: (np.array([[1850],[0],[-2000]]), np.array(South)),
        13: (np.array([[1550],[0],[-2000]]), np.array(South)),
        14: (np.array([[1250],[0],[-2000]]) ,np.array(South)),  
        15: (np.array([[750],[0],[-2000]]) ,np.array(South)), 
        16: (np.array([[450],[0],[-2000]]) ,np.array(South)),
        17: (np.array([[150],[0],[-2000]]) ,np.array(South)),
        18: (np.array([[0],[0],[-1850]]) ,np.array(West)), 
        19: (np.array([[0],[0],[-1550]]) ,np.array(West)), 
        20: (np.array([[0],[0],[-1250]]) ,np.array(West)), 
        21: (np.array([[0],[0],[-750]]) ,np.array(West)), 
        22: (np.array([[0],[0],[-450]]) ,np.array(West)), 
        23: (np.array([[0],[0],[-150]]) ,np.array(West))
