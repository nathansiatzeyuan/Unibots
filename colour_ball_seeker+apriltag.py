...
tags = {0: (np.array([[150],[0],[0]]) ,np.array(North)),
...
def process_image(pc, ser):
    lower_bound = np.array([25,100,110]) # Colour range for filtering for non-white balls
    ...
    x_distances = []
    while True:
        frame = capture_frame(pc,delay=0.1)
        ...
        for c in contours:
            ...
            if (y+(height/2)) > dispH/2 -200 and 400<(height*width)<20000: #ASSUME area=100x100
